...
static const arg_def_t bitdeptharg   = ARG_DEF_ENUM("d", "bit-depth", 1,
                                                    "Bit depth for codec "
                                                    "8 or 10 or 12. ",
                                                    bitdepth_enum);
#endif
...
static const arg_def_t *svc_args[] = {
  ...
  &min_bitrate_arg,   &max_bitrate_arg,
#if CONFIG_VP9_HIGH
  &bitdeptharg,
#endif
  NULL
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &frames_arg, argi)) {
      ...
      switch (enc_cfg->g_bit_depth) {
        case VPX_BITS_8:
          enc_cfg->g_in_bit_depth = 8;
          enc_cfg->g_profile = 0;
          break;
        case VPX_BITS_10:
          enc_cfg->g_in_bit_depth = 10;
          enc_cfg->g_profile = 2;
          break;
         case VPX_BITS_12:
          enc_cfg->g_in_bit_depth = 12;
          enc_cfg->g_profile = 2;
          break;
        default:
...
#if CONFIG_VP9_HIGH
  if (!vpx_img_alloc(&raw, enc_cfg.g_in_bit_depth == 8 ? VPX_IMG_FMT_I420 :
                                                         VPX_IMG_FMT_I42016,
